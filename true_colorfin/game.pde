...
void gameClicks() {
  ...
  if     (touchingRectg(400, 0, 400, 800) && same ==true) {  //(mouseX> width/2 && mouseX < width && mouseY> 0 && mouseY < height && same == true) {
    score = score+1;

    yes.rewind();
    yes.play();
  } else if (touchingRectg(0, 0, 400, 800) && same ==false) {    //(mouseX >0 && mouseX < width/2 && mouseY >0 && mouseY< height && same == false) {
    score=score+1;
    yes.rewind();
    yes.play();
  } else if    (touchingRectg(400, 0, 400, 800) && same ==false) {     //    (mouseX> width/2 && mouseX < width && mouseY> 0 && mouseY < height && same == false) {
    lives=lives-1;
    no.rewind();
    no.play();
  } else if      (touchingRectg(0, 0, 400, 800) && same ==true) {                       //(mouseX >0 && mouseX < width/2 && mouseY >0 && mouseY< height && same == true) {
    lives=lives-1;
    no.rewind();
    no.play();
  }
